...
1. What are the inputs to the control module?
   ...
   ```
   Opcode, Funct3, Funct7, Zero Flag, Busy Flag, IntRq

   ```

3. What are the outputs of the control module? i.e. what control signals are generated?
   ```
   ImmEn, ImmSel, IRLd, ALd, BLd, ALUOp, ALUEn, RegEn, RegSel, RegWr, MALd, MemWr, MemEn
   ```

4. Implement the control as a ROM. In task 3, you will fill the ROM with different microinstructions to execute different instructions.
...
1. What does the microsequencer do?
   ```
   Controls the execution of microinstructions
   ```

2. What are the inputs to the microsequencer?
   ```
   Opcode, Funct3, Funct7, and current state
   ```
